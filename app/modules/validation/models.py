...
never a copy of it: it carries the back-reference needed to locate the row
(`source_file` + `source_row`, or `order_id`) and nothing that identifies a
customer. `governance/core/04_SECURITY_RULES.md` §6 makes purchase price and
margin sensitive; `governance/product/17_DATA_GOVERNANCE_PRIVACY.md` makes
customer name/phone/address sensitive. A review queue that leaks either would
be a new exposure surface built by the very task meant to add safety
(CHECK-110-17).
...
class ReviewItem:
    """One finding. `affected_count` is 1 for a per-row finding and N for an
    aggregate one (DEC-128 §1) — a reader can always tell how much data sits
    behind a single line of the queue."""
