...
## Chỉ ĐỌC, và chỉ hai nhánh — `inv` KHÔNG BAO GIỜ được hỏi tới
...
**Một `board` rỗng KHÔNG được ghi thành một danh mục rỗng.** RTDB trả `null`
cho cả "nhánh không tồn tại" lẫn "nhánh rỗng", nên từ dây không phân biệt
được "danh mục thật sự trống" với "sai URL / sai nhánh / mất quyền đọc". Một
...
# Dùng lại ĐÚNG đường mạng + đường credential của công cụ capture đã review,
# thay vì chép lại: một `method="GET"` duy nhất, một `TRACKING_RTDB_TOKEN`
# duy nhất, một quy tắc bất biến `INV-11` duy nhất trong cả repo.
...
def build_capture(
    ...
    *,
    ...
    if not board:
        return failed(
            f"{EMPTY_SOURCE_NOT_ASSERTABLE}: nhánh {BOARD_NODE!r} rỗng hoặc "
            "vắng mặt. RTDB trả cùng một `null` cho 'nhánh không tồn tại' và "
            "'nhánh rỗng', nên KHÔNG kết luận được đây là một danh mục thật "
            "sự trống — người vận hành phải xác nhận URL/nhánh/quyền đọc."
        )
    try:
