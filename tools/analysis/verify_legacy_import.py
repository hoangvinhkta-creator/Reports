...
``Summary 2025``, ``DataChart 2026`` với giá trị đã lưu. Script KHÔNG sửa
gì — nó chỉ đọc.
...
``matched>0 mismatched=0``. Một báo cáo thiếu hẳn kỳ 2025 mà vẫn "khớp
100%" là loại bằng chứng tệ hơn không có bằng chứng. Vì vậy vòng lặp
Summary duyệt từ EXCEL → DB (FIND-PRA001-R01).
...
class VerificationResult:
    ...
    @property
    def ok(self) -> bool:
        return not self.mismatches and not self.summary_unaccounted_rows
