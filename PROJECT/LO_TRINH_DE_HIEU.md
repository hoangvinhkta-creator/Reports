...
| 🟡 | 11a. TASK-105B — Xây phần đọc bảng giá (nền cho bước đọc lịch sử giá) | Bước 11b (đọc lịch sử giá NCC) cần phần này làm nền trước — không thể xây sau | C | **Đã viết code xong, đã qua soát xét độc lập — PASS (2026-08-28).** Hai phiên soát xét độc lập chạy song song (không biết về nhau) đều tự kiểm tra lại từ đầu và cùng kết luận PASS, không có lỗi nghiêm trọng nào; một phiên riêng đã đối chiếu cả hai kết quả lại với nhau cho khớp (xem `docs/reviews/TASK-105B-INDEPENDENT-REVIEW-RECONCILIATION.md`). Còn hai việc trước khi tính là "xong hẳn": (1) một phiên **chốt đóng dấu (Freeze)** chính thức — chưa làm trong phiên này, (2) cần bảng giá nhập THẬT của công ty để nạp thử (hiện chưa có, không phải lỗi code). Có ghi lại vài điểm nên vá thêm sau này (ví dụ: giá trị số học đặc biệt như "vô cực" chưa bị chặn) — không chặn bước tiếp theo vì phần đọc bảng giá thật chưa được nối vào chạy thật |
| ⬜ | 11b. TASK-105C — Đọc giá nhập (từ lịch sử giá NCC) | Không có giá nhập thì không tính được lợi nhuận; đây là nút thắt còn lại | C | **Chủ dự án đã chốt xong toàn bộ, kể cả hai câu hỏi nhỏ.** Thiết kế kỹ thuật đầy đủ đã có (`docs/tasks/TASK-105C-historical-vendor-price-provider.md`). Chờ bước 11a (ngay trên) xong hẳn (soát xét độc lập PASS) rồi mới viết code bước này — 11b xây trực tiếp trên nền của 11a. Còn một việc khác cần làm song song: bảng dịch tên hàng ↔ mã sản phẩm (chưa có, chưa chặn việc bắt đầu code, nhưng chặn việc có số thật thay vì để trống hàng loạt) |
