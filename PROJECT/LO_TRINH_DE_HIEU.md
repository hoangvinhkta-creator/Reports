...
| 🟡 | 11a. TASK-105B — Xây phần đọc bảng giá (nền cho bước đọc lịch sử giá) | Bước 11b (đọc lịch sử giá NCC) cần phần này làm nền trước — không thể xây sau | C | **Đã viết code xong, tự kiểm tra đầy đủ trong phiên này (2026-08-28)** — 33 test mới, toàn bộ PASS, không ảnh hưởng gì tới phần đã chạy trước đó (0 thay đổi ở 4 file lõi, số liệu Golden kiểm tra lại y hệt). Còn hai việc trước khi tính là "xong": (1) một phiên **soát xét độc lập khác** phải xác nhận lại (bắt buộc theo quy trình cho việc rủi ro cao — sai ở đây ảnh hưởng lương), (2) cần bảng giá nhập THẬT của công ty để nạp thử (hiện chưa có, không phải lỗi code) |
